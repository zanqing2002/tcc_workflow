...
const PROVIDER_INFORMATION = process.env.PROVIDER_INFORMATION;
...
function fetchAppHostIdsXsAppNameServicesMap() {
    return new Promise((resolve) => {
        ...
        try {
            ...
            if (SAP_CONTENT_PROVIDER || PROVIDER_INFORMATION) {
        ...
        //TODO: is this relevant for saas_approuter scenario (this is important now only for Apollo - not CFLP)
        try {
            ...
            //TODO: is this relevant for saas_approuter scenario if yes then there should be UAA used there
        ...
        //TODO: is this relevant for saas_approuter scenario, will we support also Business Services bounded to content deployer
        //TODO: the BS will also be accessible fro CFLP approuter?
...
function buildContentProviderMetadata({providerInformation, portalServiceCredentials, uaaServiceCredentials, xsappname, appRouterUrl, tenantHostPattern, isExternal, exposedContent}) {
    // in SAAS_APPROUTER scenario the id will be undefined
    return {
        isExternalContentProvider: (!!isExternal).toString(),
        blueBoxCredData: {
        ...
        providerData: {
            providerType: 'cf',
            data: {
                id: providerInformation.appName,
                deprecatedId: providerInformation.xsappname && providerInformation.xsappname.replace(/(!|\|.*)/g, ""),
                url: appRouterUrl,
                tenantHostPattern: tenantHostPattern,
                xsappname: xsappname,
                category: providerInformation.category || "paas",
                translations: [{
                    locale: '',
                    textDictionary: {
                        title: providerInformation.display_name || providerInformation.title || providerInformation.appName || '',
...
function fetchBlueBoxMetadata() {
    return new Promise((resolve) => {
            ...
            if (SAP_LOCAL_CONTENT_PROVIDER) {
                try {
                    ...
                    let providerMetaData = {
                        providerInformation: contentProviderCred,
                        xsappname: uaaServiceCredentials.xsappname,
                        exposedContent: "embedded"
                    };
                    result = buildContentProviderMetadata(providerMetaData);
                } catch (err) {
                ...
                try {
                    ...
                    let providerMetaData = {
                        providerInformation: saasServiceCredentials,
                        portalServiceCredentials: portalServiceCredentials,
                        xsappname: xsappname,
                        exposedContent: "embedded",
                        isExternal: true
            ...
            } else if (PROVIDER_INFORMATION) {
                try {
                    const uaaServiceCredentials = xsenv.cfServiceCredentials({tag: 'xsuaa'});
                    let providerMetaData = {
                        providerInformation: JSON.parse(PROVIDER_INFORMATION),
                        xsappname: uaaServiceCredentials.xsappname,
                        exposedContent: "SAAS_APPROUTER"
                    };
                    result = buildContentProviderMetadata(providerMetaData);
                } catch (err) {
                    console.error('fetchBlueBoxMetadata - PROVIDER_INFORMATION, exception was thrown: ', err.message);
                    throw err;
                }
            } else {
...
class DeployUtils {
     ...
     */
    static async deploy() {
        console.log(`Deploy flow - SAP_CONTENT_PROVIDER = ${SAP_CONTENT_PROVIDER}, SAP_LOCAL_CONTENT_PROVIDER = ${SAP_LOCAL_CONTENT_PROVIDER}, PROVIDER_INFORMATION = ${PROVIDER_INFORMATION}`);
